...
let cHandler = fs.existsSync('./ai/c.json') ? JSON.parse(fs.readFileSync('./ai/c.json', 'utf-8')) : {},
    brain = fs.existsSync('./ai/brain.json') ? JSON.parse(fs.readFileSync('./ai/brain.json', 'utf-8')) : {},
...
const learn = async (sentence) => {
    ...
    console.log("[AI] Performing anti corruption check... " + cHandler?.t)
...
const gainSentience = async (channel, messageWordLength = Math.floor(Math.random() * 12) + 3, messageChannel = channel) => {
    try {
        ...
        return await messageChannel.send(getMessages(messageWordLength)).catch(err => console.log(err));
    } catch (error) {
